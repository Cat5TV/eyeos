...
/* Change Log:
 *  2014-1028 ! [djs]
 *    * Change mysql_connect() to mysqli
 *    - Removed bottom php terminator to stay compliant to standards
 *
 */
...
function getContent() {
  if (isset($_POST['mysqlhost'])) {
    ...
    if (mysqli_connect_errno()) {  // if ($link->connect_errno) {   //if(!$link) {
      echo '<p>Unable to connect to databse: ' . mysqli_connect_error() . '</p>';
      echo "<ul><li>Host: '" . $_POST['mysqlhost'] . "'</li><li>User: '" . $_POST['mysqluser'] . "'</li><li>Pass: '" . $_POST['mysqlpass'] . "'</li></ul>";
      echo '<p><a href="index.php?step=configuration">Click here to go back</a></p>';
      return;
    }

    //if (!mysql_select_db($_POST['mysqldb'], $link)) {
    if (!$link->select_db($_POST["mysqldb"])) {
      echo '<p>Unable to select databse: ' . $link->connect_errno . '</p>';   //. mysql_error() . '</p>';
      echo '<p><a href="index.php?step=configuration">Click here to go back</a></p>';
      return;
    }
    ...
    foreach ($files as $file) {
      ...
      foreach ($content as $line) {
        ...
        if (substr($line, -1, 1) == ';') {
          ...
          $link->query($buffer);   //mysql_query($buffer, $link);
    ...
    $link->query($sql); // mysql_query($sql, $link);
    $link->close();
